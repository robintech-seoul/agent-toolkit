...
- (v4.2) 설계 반려 후 재진입: `design-loop.sh` 1라운드가 기존 DESIGN.md 를 새로 쓰지 않고 빠진 항목만 보완한다(반려 수정본 보존,
  빠진 것이 없으면 모델 호출 생략). 리뷰 루프는 원장에 발견이 있으면 첫 라운드부터 **델타**로 돌아 열린 must 를 재판정한다
  (반려 수정 diff 가 입력). 라운드 번호는 기존 산출물 뒤에 이어 붙는다. 커맨드의 `$ARGUMENTS` 를 인용해 아이디어·사유 속
  `[a|b]`·괄호·`<x>` 가 셸에서 풀리지 않는다
